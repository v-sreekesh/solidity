...
)
{
	...
	for (auto const& parameter: parameters)
	{
		...
		if (!parameter.matchesBytes(byteRange) ||
			// Fixed point values may look exactly the same, but its actual value may be different if
			// it uses a different fixed point type (e.g. 0xb is 11, but could also be 1.1).
			// That's why we always enforce reformatting fixed point types here.
			// Note: formatting fixed point value will take specific fixed point type into account.
